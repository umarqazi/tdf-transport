                    ...
                    <table class="table table-striped table-bordered tbl-internal" >
                      <tr>
                        ...
                        @if($authUser->type==Config::get('constants.Users.Manager'))
                          <td><input type="checkbox" @if(strtotime($key)!=strtotime(date('d-M-Y'))) class="deliveryCheckbox" @endif name="delivery_id[]" value="{{$dayDelivery['id']}}" {{($dayDelivery['status']==Config::get('constants.Status.Active') || $dayDelivery['status']==Config::get('constants.Status.Return') || $dayDelivery['status']==Config::get('constants.Status.Delivered'))?'checked disabled':''}} class="" @if(strtotime($key) != strtotime(date('d-M-Y')) && $key!=date('d-M-Y', strtotime($nextDate))) disabled @endif>
                          </td>
                        @endif
                      </tr>
